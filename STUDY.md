...
A: 没有正式跑过 Lighthouse。估算的话：性能分可能在 60-75 之间（主要瓶颈是商品列表页一次性渲染 20 张卡片，虽然有懒加载但布局偏移大），可访问性 80+，SEO 70+（SPA 对 SEO 天然不友好）。改进方向：1）商品列表做虚拟滚动（react-window），只渲染可视区域的卡片；2）图片用 `<picture>` 标签提供 WebP 格式；3）加骨架屏（Skeleton）而不是 spinner 过渡；4）预加载关键资源。

> **追问**：「Lighthouse 的 TBT（Total Blocking Time）和 LCP（Largest Contentful Paint）在商品详情页上主要由什么决定了？」

A: LCP 在商品详情页上主要由**商品主图加载时间**决定——如果图片没做好懒加载或没给预宽高，浏览器需要等图片完全下载才能确定布局，LCP 会很高。优化：给 `<img>` 加 `width` 和 `height` 属性（或 CSS aspect-ratio），图片占位空间预先分配好，LCP 不依赖图片加载。TBT 主要由**JS 执行时间**决定——商品详情页加载后 React 组件渲染、商品数据 fetch 回来的 setState 引起的 re-render，这些都可能阻塞主线程。优化：代码分割（React.lazy）、耗时计算放到 Web Worker。目前代码里还没做这些优化。
